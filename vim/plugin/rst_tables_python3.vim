...
def join_rows(rows, sep='\n'):
	...
	return [ x for x in map(lambda lines: sep.join(lines), output)]
...
def parse_table(raw_lines):
	...
	lines = [x for x in map(lambda row_string: join_rows(map(split_table_row, row_string)),
				row_partition)]
...
def split_row_into_lines(row):
	row = [x for x in map(lambda field: field.split('\n'), row)]
...
def draw_table(table, manual_widths=None):
	...
	sep_col_widths = [y for y in map(lambda x: x + 2, col_widths)]
...
if !exists("no_plugin_maps") && !exists("no_rst_table_maps")
	if !hasmapto('ReformatTable(')
		noremap <silent> <leader><leader>c :python3 ReformatTable()<CR>
	endif
	if !hasmapto('ReflowTable(')
		noremap <silent> <leader><leader>f :python3 ReflowTable()<CR>
	endif
endif
